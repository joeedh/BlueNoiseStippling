...
export class BlueNoise {
  ...
  step_b(custom_steps?: number, skip_points_display?: boolean): void {
  ...
  _inner_loop(
    ...
    if (config.DITHER_BLUE)
      colors.ditherSampler.seed(this.getPixelSeed(1.0 - threshold));
  ...
  step_smask(custom_steps?: number): void {
  ...
  step_diffusion(custom_steps: number | undefined, cur: number): number {
