...
const VERSION = 0.003;
const eval_rets = new util.cachering(() => [0, 0], 64);

class BinaryEncoder extends Array<number> {
  buffer: ArrayBuffer;
  u8view: Uint8Array;
  u16view: Uint16Array;
  i32view: Int32Array;
...
export class MaskPoint {
  ...
  constructor(
    ...
    this.startx = startx as number;
    this.starty = starty as number;
  ...
  fromBinary(decoder: BinaryDecoder): this {
    ...
    if (decoder.version < 0.003) {
      ...
      let ftot = this.fieldlen,
        totpoint = this.offsets.length / ftot;
  ...
  encode16(encoder: BinaryEncoder): this {
    let min = 1e17,
      max = -1e17;
  ...
  encode32(encoder: BinaryEncoder): void {
    encoder.int32(this.offsets.length);
  ...
  decode16(decoder: BinaryDecoder): this | void {
    let len = decoder.int32();
  ...
  loadJSON(obj: Record<string, unknown>): this {
    for (let k in obj) {
      (this as Record<string, unknown>)[k] = obj[k];
    }
  ...
  compress(): void {
    ...
    for (let i = fieldlen; i < offs.length - fieldlen; i += fieldlen) {
      let ax = offs2[offs2.length - fieldlen],
        ay = offs2[offs2.length - fieldlen + 1];
      let bx = offs[i],
        by = offs[i + 1];
      let cx = offs[i + fieldlen],
        cy = offs[i + fieldlen + 1];
  ...
  linear_nonuniform_search(t: number): number {
    let offs = this.offsets;
  ...
  evaluate_linear_nonuniform(t: number): number[] {
    let offs = this.offsets;
    ...
    if (a > 0 && offs[a + NT] > t) {
      //paranoia check for off-by-one errors
      a -= NTOT;
    }
  ...
  evaluate_linear_uniform(t: number): number[] {
    ...
    ret[1] =
      this.offsets[a + 1] + (this.offsets[b + 1] - this.offsets[a + 1]) * s;
...
export class PointSet {
  ...
  setInverseToneCurve(curve: number[]): void {
    this.inverse_tone_curve = curve.slice(0, curve.length);
  }

  evalInverseTone(s: number): number {
  ...
  fromBinary(buf: string): this {
    ...
    if (version > 0.001) {
      ...
      let curve: number[] = (this.inverse_tone_curve = []);
  ...
  loadJSON(obj: { points: MaskPoint[]; blocksize: number }): void {
    this.points = obj.points;
    this.blocksize = obj.blocksize;

    // NOTE: original JS compares `i` against the array `this.points` (not
    // `.length`); preserved verbatim. Cast keeps that exact runtime behavior.
    for (let i = 0; i < (this.points as unknown as number); i++) {
      let p = new MaskPoint();
      p.loadJSON(obj as unknown as Record<string, unknown>);
