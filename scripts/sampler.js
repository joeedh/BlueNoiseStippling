...
export function fdataToInternal(fdata) {
...
export function fdataFromInternal(fdata) {
...
export function histEqualize(image) {
  for (let i = 0; i < 4; i++) {
...
export function histEqualize_intern(image, factor) {
...
export function fdataToData8(fdata, idata) {
  let inv = 1.0/255;
  let internal_to_rgb = colors.internal_to_rgb;
...
export function debandImage(image) {
  ...
  //for (let i=0; i<downSteps; i++) {
  if (downSteps > 0)
    image2 = upsampleImage(image2, 1<<downSteps);
  //}
...
export function upsampleImage(image, factor = 2) {
...
export function downsampleImage(image) {
...
export function sampler(x, y, size, rad, no_filter, use_debanded) {
