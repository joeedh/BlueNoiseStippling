...
// `_appstate` is a bare global installed on window at runtime (see appstate.ts).
// eslint-disable-next-line @typescript-eslint/no-explicit-any
declare const _appstate: any;

/**
 * A color is stored as a plain array of channel values (e.g. [r, g, b] or
 * [r, g, b, a]).  Many helpers also accept the array-like {@link Vector3}.
 */
export type Color = number[];
/** Anything that can be read by numeric index as a color (arrays or Vector3/4). */
...
function getDitherFac() {
  ...
  return config.DITHER_RAND_FAC;
}

export class DitherSampler extends Array<number> {
  cur: number;
...
let rgb_to_hsv_rets = new util.cachering<Color>(() => [0, 0, 0], 64);

export function rgb_to_hsv(r: number, g: number, b: number): Color {
  let computedH = 0;
  let computedS = 0;
  let computedV = 0;
...
let hsv_to_rgb_rets = new util.cachering<Color>(() => [0, 0, 0], 64);

export function hsv_to_rgb(h: number, s: number, v: number): Color {
  let c = 0,
    m = 0,
    x = 0;
  ...
  function RgbF_Create(r: number, g: number, b: number): Color {
    ret[0] = r;
    ret[1] = g;
    ret[2] = b;
...
export var closest_map: Int32Array | undefined = undefined;
export var closest_size: number | undefined = undefined;
export var next_closest_map: Int32Array | undefined = undefined;
...
export function xyz_to_intensity(x: number, y: number, z: number): number {
...
export function xyz_to_saturation(x: number, y: number, z: number): number {
  let avg = (x + y + z) / 3.0;
...
export function rgb_to_xyz(r: number, g: number, b: number): Vector3 {
  let var_R = r;
  let var_G = g;
  let var_B = b;
...
export function xyz_to_rgb(X: number, Y: number, Z: number): Vector3 {
  ...
  if (var_R > 0.003130807) var_R = 1.055 * Math.pow(var_R, 1.0 / 2.4) - 0.055;
  else var_R = 12.92 * var_R;

  if (var_G > 0.003130807) var_G = 1.055 * Math.pow(var_G, 1.0 / 2.4) - 0.055;
  else var_G = 12.92 * var_G;

  if (var_B > 0.003130807) var_B = 1.055 * Math.pow(var_B, 1.0 / 2.4) - 0.055;
...
export function lab_to_xyz(L: number, a: number, b: number): Vector3 {
  L *= 100.0;
  a *= 100.0;
  b *= 100.0;
...
export function xyz_to_lab(X: number, Y: number, Z: number): Vector3 {
  ...
  let L = 1.16 * var_Y - 0.016;
  let a = 5.0 * (var_X - var_Y);
  let b = 2.0 * (var_Y - var_Z);
...
export function xyz_colordis(c1: ColorLike, c2: ColorLike): number {
...
function lab_to_labch(L: number, a: number, b: number): Vector3 {
  L *= 100.0;
  a *= 100.0;
  b *= 100.0;
...
export function labch_to_lab(L: number, c: number, h: number): Vector3 {
...
export function rgb_to_lab(r: number, g: number, b: number): Vector3 {
...
export function lab_to_rgb(l: number, a: number, b: number): Vector3 {
...
export function lab_to_intensity(l: number, a: number, b: number): number {
  return l;
}

export function lab_to_saturation(l: number, a: number, b: number): number {
  let labch = lab_to_labch(l, a, b);
  return labch[1];
}

export function rgb_to_labch(r: number, g: number, b: number): Vector3 {
...
export function rgb_to_cmyk(r: number, g: number, b: number): Vector3 {
...
export function labch_to_rgb(l: number, c: number, h: number): Vector3 {
...
export function gen_closest_map(size?: number): Int32Array {
  if (size === undefined) size = config.LOW_RES_CUBE ? 20 : 40;
...
export function closest_color_fast(
  clr: ColorLike,
  nextout?: number[],
  fr = 0,
  fg = 0,
  ...
  // gen_closest_map() (called above or previously) always assigns all three maps.
...
let _last_map: string | undefined = undefined;

export var colors_used: number[] = new Array();
export var lab_colors: Color[] = [];

let palrand = new util.MersenneRandom(0);

export function palette_from_image(maxcolor = config.PAL_COLORS * 24): void {
  colors.length = 0;
...
export function gen_colors(): void {
  lab_colors.length = 0;

  //generate rgb palette
  if (config.IMAGE_PALETTE && _appstate.image && _appstate.image.data.fdata) {
    palette_from_image();
  } else {
    gen_colors_intern();
  }
...
export function gen_colors_intern(): void {
  colors.length = 0;

  //XXX
  if (
    config.TRI_MODE ||
    config.RASTER_MODE === RASTER_MODES.PATTERN ||
    config.RASTER_MODE === RASTER_MODES.DIFFUSION
  ) {
    colors.push([1, 1, 1]);
  }

  if (config.BG_PALETTE) {
    colors.push([0, 0, 0]);
    gen_closest_map();
    return;
  }

  if (config.SIMPLE_PALETTE) {
    colors.push([0, 0, 0]);
  ...
  //evil!
  let totpurple = config.ALLOW_PURPLE ? base : 0;
...
export function image_palette(maxcolor?: number): Color[] {
  gen_colors();

  if (maxcolor === undefined) maxcolor = 8;
  ...
  let dx = ~~(cw / config.DIMEN);
  let dy = ~~(ch / config.DIMEN);
  ...
  function cidx(r: number, g: number, b: number): number {
    r = ~~(r * csize);
    g = ~~(g * csize);
    b = ~~(b * csize);
  ...
  function weight(c: [number, number]): number {
    let clr = colors[c[0]];
...
function safe_sqrt(n: number): number {
  return n > 0 ? Math.sqrt(n) : 0;
}

export function lab_saturation(l: number, a: number, b: number): number {
  let ch = lab_to_labch(l, a, b);

  return ch[1];
}

export function lab_colordis(c1: ColorLike, c2: ColorLike): number {
  if (1) {
  ...
  let cc1 = lab_to_labch(c1[0], c1[1], c1[2]);
  let cc2 = lab_to_labch(c2[0], c2[1], c2[2]);
  ...
  let dL = cc1[0] - cc2[0];
  let L_ = (cc1[0] + cc2[0]) * 0.5;
  let C_ = (cc1[1] + cc2[1]) * 0.5;
  let C_7 = pow(C_, 7.0);

  let da1 = l1[1] + 0.5 * l1[1] * (1.0 - sqrt(C_7 / (C_7 + twentyfive7)));
  let da2 = l2[1] + 0.5 * l2[1] * (1.0 - sqrt(C_7 / (C_7 + twentyfive7)));
  let dC1 = sqrt(cc1[1] * cc1[1] + cc1[2] * cc1[2]);
  let dC2 = sqrt(cc2[1] * cc2[1] + cc2[2] * cc2[2]);
  let dC_ = (dC1 + dC2) * 0.5;
  ...
  let dhdeg: number;
  // NOTE: the original passed two args to Math.abs; the second is ignored at
  // runtime, so abs(h1deg) is exactly equivalent.
  let hdif = abs(h1deg);
  if (hdif < 180) {
    dhdeg = h2deg - h1deg;
  ...
  T += -0.2 * cos((Math.PI * (4 * H_ - 63)) / 180);

  let SL =
    1 + (0.015 * (L_ - 50.0) * (L_ - 50.0)) / sqrt(20 + (L_ - 50) * (L_ - 50));
  let SC = 1 + 0.045 * C_;
  let SH = 1 + 0.015 * C_ * T;
...
export function colordis_not_diffusing(c1: ColorLike, c2: ColorLike): number {
  ...
  dis =
    (Math.abs(dr) * w1 + Math.abs(dg) * w2 + Math.abs(db) * w3) /
    (w1 + w2 + w3);
...
export function colordis_simple(c1: ColorLike, c2: ColorLike): number {
  return colordis(c1, c2);
...
export function rgb_colordis(c1: ColorLike, c2: ColorLike): number {
  let w1 = 1,
    w2 = 0.8,
    w3 = 0.6;
  w1 = w2 = w3 = 1.0;
...
export function old_rgb_colordis(c1: ColorLike, c2: ColorLike): number {
...
let _closest_color2_tmp: Color = [0, 0, 0];
let closest_color2_rets = new util.cachering<Color>(function () {
  return [0, 0, 0];
}, 256);

export function closest_color2(
  f: number,
  c1: ColorLike,
  nextout?: number[],
  ditherfac?: number,
...
let _cc_tmp1: number[] = new Array();
let _cc_tmp2: number[] = new Array();

export function closest_color_lab(
  c1: ColorLike,
  nextout?: number[],
  ditherfac?: number,
  not_diffusing?: boolean,
): number {
  let mindis = undefined;
  let mindis2 = undefined,
    mindis3 = undefined,
    mindis4 = undefined,
    mindis5 = undefined;
  let ret = undefined;
...
export function closest_color(
  ...
  if (nextout === undefined) {
    // `colors` is non-empty when this is called, so `mini` is a real index.
    return mini!;
  }
  ...
  // `_cc_tmp1` mirrors the non-empty `colors`, so index 0 is always present.
  return _cc_tmp1[ci]!;
}
...
let barycentric_rets = new util.cachering<Color>(function () {
  return [0, 0, 0];
}, 32);
...
export function colorweight(r: number, g: number, b: number): number[] {
  let wr = 0,
    wg = 0,
    wb = 0;
  let c = _cwc,
    tot = 0;
  let ret: number[] = new Array(colors.length);
...
export function rgb_to_intensity(r: number, g: number, b: number): number {
...
let rgb_to_rgb = function (r: number, g: number, b: number): Vector3 {
...
export function rgb_to_saturation(r: number, g: number, b: number): number {
  let w1 = 1,
    w2 = 0.8,
    w3 = 0.7;
  let totw = 1.0 / (w1 + w2 + w3);
...
/** Converts three channel values into a {@link Vector3} of another color space. */
export type ChannelToVector = (a: number, b: number, c: number) => Vector3;
/** Reduces three channel values to a single scalar (intensity/saturation). */
export type ChannelToScalar = (a: number, b: number, c: number) => number;
/** Distance metric between two colors; the optional dither factor is ignored by some metrics. */
export type ColorDistance = (
  c1: ColorLike,
  c2: ColorLike,
  ditherfac?: number,
) => number;

let colorspace: ColorSpace = 0;
let spacestack: ColorSpace[] = [];

export function pushColorSpace(space: ColorSpace): void {
  spacestack.push(colorspace);
  setColorSpace(space);
}

export function popColorSpace(): ColorSpace | undefined {
...
export var rgb_to_internal: ChannelToVector;
export var internal_to_rgb: ChannelToVector;
export var internal_to_intensity: ChannelToScalar;
export var internal_to_saturation: ChannelToScalar;
export var colordis: ColorDistance;
