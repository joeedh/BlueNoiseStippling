...
let rot_rets = new util.cachering(function () {
  return [0, 0];
}, 256);
...
export class BlueNoise {
  ...
  step_b(custom_steps, skip_points_display) {
    let raster_image = this.raster_image;
    ...
    let grid = this.errgrid;
    let size = this.gridsize;
    let iw = DIMEN, ih = DIMEN;

    let steps = custom_steps ? custom_steps : window.STEPS;
    let cw = this.mask.width;
    let ch = this.mask.height;
    ...
    let mask = this.mask.data.data;
    let mscale = SMALL_MASK ? 1 : 4;
    ...
    let do_cmyk_raster = RASTER_MODE === RASTER_MODES.CMYK;
    ...
    for (let si = 0; si < steps; si++, this.cur++) {
      ...
      let clr = this.sampler((x - 0.5)*2, (y - 0.5)*2, size, 1.0);
      if (clr[0] < 0) continue;
      ...
      let rix = ~~(x*raster_image.width*0.99999), riy = ~~(y*raster_image.height*0.99999);
      ...
      let fac = 0, fac2 = 0, fac3 = 0;
      let tot = 0, tot2 = 0, tot3 = 0;
      let mx, my;

      for (let j = 0; j < mscale*mscale; j++) {
        let offx = j%mscale, offy = ~~(j/mscale);
  ...
  _inner_loop(args, refine) {
    ...
    let raster_image = this.raster_image;
    let size = this.gridsize;
    let iw = DIMEN, ih = DIMEN, cw = iw, ch = ih;
    let start_r = this.start_r;
  ...
  step_diffusion(custom_steps, cur) {
    ...
    for (let si = 0; si < steps; si++, cur++) {
      ...
      let rok = 0, gok = 0, bok = 0, fok = 0;
      let finalth = 0.0;

      let fr = clr[0], fg = clr[1], fb = clr[2];

      let ci = colors.closest_color_fast(clr);

      let rasterfac = raster_image !== undefined ? ~~Math.ceil(raster_image.width/this.dimen) : 1;
      ...
      if (DITHER_COLORS) {
        ...
        for (let fi = 0; fi < fil.length; fi++) {
          for (let fj = 0; fj < fil[fi].length; fj++) {
            ...
            let fmul = this.filter.wrand*(this.random() - 0.5);
        ...
        let sat = Math.abs(1.0 - clr2[0]) + Math.abs(1.0 - clr2[1]) + Math.abs(1.0 - clr2[2]);
        sat /= 3.0;
      }

      let do_raster = RASTER_IMAGE && rdata !== undefined;
      if (do_raster) {
        let clr2 = colors.colors[ci];
  ...
  step_cmyk_color_mask(custom_steps, skip_points_display) {
    ...
    for (let si = 0; si < steps; si++, this.cur++) {
      ...
      let rx = this.cur%size;
      let ry = ~~(this.cur/size);
      if (flip) {
        rx = size - rx - 1;
      }
    ...
    for (let si = 0; si < steps; si++, this.cur++) {
      ...
      switch(lvl) {
        case 0:
          ...
          break;
        case 1:
          let tf = Math.max(clr[1], clr[2]);
          f *= 1.0-clr[0];
          //f = (1.0-clr[0])*(1.0-tf) + clr[0]*tf;

          break;
        case 2:
          let tf = Math.max(clr[0], clr[2]);
          f *= 1.0-clr[1];
          //f = (1.0-clr[1])*(1.0-tf) + clr[1]*tf;
          break;
        case 3:
          let tf = Math.max(clr[1], clr[0]);
          f *= 1.0-clr[2];
      ...
      for (let i = 0; !stop && i < wid; i++) {
        for (let j = 0; !stop && j < wid; j++) {
          ...
          if (rotmask) {
            ...
            /*
            let rt = dorot(cw-1-i, j, ix, iy, Math.PI/4.0);
            ix3 = rt[0];
            iy3 = rt[1];

            let rt = dorot(j, cw-1-i, ix, iy, -Math.PI/5.0);
            ix4 = rt[0];
            iy4 = rt[1];

            let rt = dorot(j, i, ix, iy, Math.PI/3.0);
            ix5 = rt[0];
            iy5 = rt[1];
            //*/
          } else {
          ...
          let aok2 = mask[idx2] > 1 || SMALL_MASK;
          let aok3 = mask[idx3] > 1 || SMALL_MASK;
          let aok4 = mask[idx4] > 1 || SMALL_MASK;
          let aok5 = mask[idx5] > 1 || SMALL_MASK;
      ...
      if (!SMALL_MASK && ok && !GRID_MODE) {
        ...
        let sf = XLARGE_MASK ? 0.25 : 0.4;//RASTER_IMAGE ? 0.8 : 0.501;
        sf = 2.0/mscale;
      ...
      if (DITHER_COLORS && (RASTER_IMAGE && RASTER_MODE !== RASTER_MODES.CMYK)) {
        ...
        if (CORRECT_FOR_SPACING) {
          ...
          let sat2 = Math.abs(avg - clr[0]) + Math.abs(avg - clr[1]) + Math.abs(avg - clr[2]);
          sat2 /= 3.0;
      ...
      if (do_raster && RASTER_MODE === RASTER_MODES.DIFFUSION) {
        ...
        let rix2 = ~~((x*0.5 + 0.5001)*2*raster_image.width*0.99999),
  ...
  calc_radii_old(use_orig_cos) {
    ...
    let minrad2 = 1e17;
    let tot, avgr, minr, maxr, x1, y1, ix, iy, ix1, iy1;
  ...
  del() {
    ...
    let vcells = this.vcells = new Float64Array(plen*MAX_VCELL_SIZE);

    vcells.fill(-1, 0, vcells.length);

    for (let i = 0; i < tris.length; i += 3) {
      for (let j = 0; j < 3; j++) {
        let v1 = tris[i + j], v2 = tris[i + ((j + 1)%3)];
        let v3 = tris[i + ((j + 2)%3)];
    ...
    let sortlst = new Float64Array(MAX_VCELL_SIZE);
    ...
    for (let i = 0; i < vcells.length; i += MAX_VCELL_SIZE) {
      let pi = i/MAX_VCELL_SIZE;
      x = ps[pi*PTOT], y = ps[pi*PTOT + 1];

      for (let j = 0; j < MAX_VCELL_SIZE; j++) {
  ...
  relax3() {
    ...
    let GN = 0, GDX = 1, GDY = 2, GSUM = 3, GNUM = 4, GW = 5, GTOT = 6;
  ...
  relax() {
    ...
    let GW = 0, GDX = 1, GDY = 2, GSUM = 3, GTOT = 4;
    let size = this.dimen*2;
    ...
    for (let i = 0; i < ps.length; i += PTOT) {
      ...
      for (let j = 0; j < offs.length; j++) {
        ...
        let dx = offs[j][0]/size, dy = offs[j][1]/size;
