...
/**
 * Pixel buffer shape used throughout the sampling pipeline. Images may carry
 * either an 8-bit `data` buffer (as on `ImageData`) and/or a float `fdata`
 * buffer; some code paths read whichever is present.
 */
export interface ImageLike {
  width: number;
  height: number;
  data?: Uint8Array | Uint8ClampedArray;
  fdata?: Float32Array | Float64Array;
}

/** Numeric pixel buffer (typed array) accepted by the per-pixel helpers. */
type PixelBuffer = Uint8Array | Uint8ClampedArray | Float32Array | Float64Array;

export function fdataToInternal(fdata: PixelBuffer): void {
  let rgb_to_internal = colors.rgb_to_internal;
  let inv = 1.0 / 255.0;

  for (let i = 0; i < fdata.length; i += 4) {
    let c = rgb_to_internal(
      fdata[i] * inv,
      fdata[i + 1] * inv,
      fdata[i + 2] * inv,
    );
...
export function fdataFromInternal(fdata: PixelBuffer): void {
  let internal_to_rgb = colors.internal_to_rgb;
  let inv = 1.0 / 255.0;

  for (let i = 0; i < fdata.length; i += 4) {
    let c = internal_to_rgb(
      fdata[i] * inv,
      fdata[i + 1] * inv,
      fdata[i + 2] * inv,
    );
...
export class Histogram extends Array<number> {
  /** Snapshot of the non-cumulative distribution, taken in `finish()`. */
  df?: number[];
...
/**
 * Result of a single histogram-equalization pass. The produced image carries a
 * `heigth` key (a pre-existing typo in the original code) instead of `height`;
 * preserved verbatim to keep runtime output identical.
 */
interface HistEqualizeResult {
  image2: ImageLike;
  err: number;
}

export function histEqualize(image: ImageLike): ImageLike {
  for (let i = 0; i < 4; i++) {
    // `!` preserves original behavior: histEqualize_intern can return undefined
    // (empty histogram), in which case the original code throws on destructure.
    let { image2, err } = histEqualize_intern(image, 0.4)!;
...
export function histEqualize_intern(
  image: ImageLike,
  factor: number,
): HistEqualizeResult | undefined {
  image = Object.assign({}, image);

  if (image.fdata === undefined) image.fdata = new Float32Array(image.data!);
  ...
  return {
    // Cast forced: the runtime object uses the misspelled key `heigth` (and so
    // has no `height`), which is preserved verbatim from the original code.
    image2: {
      width: image.width,
      heigth: image.height,
      fdata: fdata2,
...
export function debandImage(image: ImageLike): ImageLike {
  ...
  for (let i = 0; i < downSteps; i++) {
    image2 = downsampleImage(image2);
  }

  console.log("Blurring");
  image2 = blurImage(image2);

  console.log("Upscaling");
  //for (let i=0; i<downSteps; i++) {
  if (downSteps > 0) image2 = upsampleImage(image2, 1 << downSteps);
  //}

  console.log("Done");

  // Both buffers are present here: image is a float image and image2 is
  // produced by the up/down/blur helpers, all of which set fdata.
  let fdata1 = image.fdata!,
    fdata2 = image2.fdata!;
  ...
  for (let i = 0; i < fdata2.length; i += 4) {
    ...
    let idx1 = (iy * image.width + ix) * 4,
      idx2 = i;

    for (let j = 0; j < 4; j++) {
      let f =
        fdata1[idx1 + j] +
        (fdata2[idx2 + j] - fdata1[idx1 + j]) * config.DEBAND_BLEND;
...
export function upsampleImage(image: ImageLike, factor = 2): ImageLike {
  let width = ~~(image.width * factor),
    height = ~~(image.height * factor);

  let fdata = new Float32Array((width * height) << 2);
  let fdata2 = image.fdata!; // upsampleImage is only called on float images
  ...
  for (let i = 0; i < fdata2.length; i += 4) {
    ...
    let ix2 = Math.min(ix + 1, width2 - 1),
      iy2 = Math.min(iy, height2 - 1);
...
export function downsampleImage(image: ImageLike): ImageLike {
...
export function copyImage(image: ImageLike): ImageLike {
  let image2: ImageLike;
...
export function blurImage(image: ImageLike): ImageLike {
  ...
  let msize = config.DEBAND_RADIUS;
    ...
    for (let step2 = 0; step2 < 2; step2++) {
      ...
      for (let iy = 0; iy < ylen; iy++) {
        ...
        for (let ix2 = 0; ix2 < xlen + (msize >> 1); ix2++) {
          ...
          if (step) i = (ix * width + iy) * 4;
          else i = (iy * width + ix) * 4;

          let ix3 = Math.min(ix2 - msize, xlen - 1);
          if (ix3 >= 0) {
            let i2;

            if (step) i2 = (ix3 * width + iy) * 4;
            else i2 = (iy * width + ix3) * 4;
...
export function sampler(
  ...
  size: number,
  rad: number,
  no_filter: number | boolean,
  use_debanded: boolean,
): number[] {
  let img: ImageLike = use_debanded
    ? window._appstate.bluenoise.dvimage
    : window._appstate.image.data;
  let ret: number[];
  ...
  const sharpen_lumin =
    config.SHARPEN_LUMINENCE && colors.internal_to_rgb === colors.lab_to_rgb;
  ...
  if (config.NO_IMAGE_FILTER) no_filter = 1;

  // img always carries either fdata or data in this pipeline.
  let pdata: PixelBuffer = (img.fdata ? img.fdata : img.data)!;
  ...
  if (no_filter) {
    ...
    if (config.TONE_CURVE !== undefined) {
      ret[0] = config.TONE_CURVE.evaluate(ret[0]);
      ret[1] = config.TONE_CURVE.evaluate(ret[1]);
      ret[2] = config.TONE_CURVE.evaluate(ret[2]);
    }

    return ret;
  }

  let filterw = img.width / window._appstate.bluenoise.gridsize;
  let filterh = img.height / window._appstate.bluenoise.gridsize;
  let filter = Math.max(filterw * 0.5, filterh * 0.5) * 1.5 + 2;

  if (isNaN(filter)) {
    console.log(
      "EEEK! 'filter' was NaN in sampler!",
      img.width,
      img.height,
      window._appstate.bluenoise.gridsize,
    );
  }
  ...
  let weights = cconst.get_sharpen_filter(fwid, config.SHARPNESS);
  ...
  for (var i = 0; i < totsample; i++) {
    ...
    if (config.TONE_CURVE !== undefined) {
      r = config.TONE_CURVE.evaluate(r);
      g = config.TONE_CURVE.evaluate(g);
      b = config.TONE_CURVE.evaluate(b);
    }
    ...
    // weights2 is assigned iff sharpen_lumin is true, so `!` is safe here.
