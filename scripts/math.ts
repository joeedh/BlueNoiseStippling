...
// Minimal constructor-with-prototype shape for the legacy init_prototype/inherit
// helpers below. These predate ES6 classes and manipulate prototypes directly.
type ProtoCtor = { prototype: Record<string, unknown> };

function init_prototype(cls: ProtoCtor, proto: Record<string, unknown>) {
  for (let k in proto) {
...
// A plain coordinate tuple. The Vector* classes extend Array<number>, so they
// are assignable to this; callers also pass bare number[] literals.
type Coord = number[];

// Structural shape of the mesh object used by the vestigial gen_circle /
// makeCircleMesh helpers. The concrete Mesh class lives in a separate (not yet
// ported / not bundled) module, so these reference it only structurally.
interface MeshVert {
  co: Coord;
}
interface MeshLike {
  make_vert(co: Vector3): MeshVert;
  make_edge(v1: MeshVert, v2: MeshVert): unknown;
  make_face_complex(loops: MeshVert[][]): unknown;
}

// `Mesh` and `__get_iter` are legacy globals from the original framework that
// makeCircleMesh / minmax_verts still reference. They are not provided by this
// project, so they are declared here purely as the external boundary they are.
declare const Mesh: new () => MeshLike;
declare function __get_iter<T>(obj: unknown): Iterator<T>;
...
export let FLOAT_MIN = -1e21;
export let FLOAT_MAX = 1e22;
...
let _static_grp_points4: Coord[] = new Array(4);
let _static_grp_points8: Coord[] = new Array(8);
...
export function get_rect_points(p: Coord, size: Coord): Coord[] {
  ...
  if (p.length === 2) {
    ...
    throw (
      "get_rect_points has no implementation for " +
      p.length +
      "-dimensional data"
    );
  }
...
let $vs_simple_tri_aabb_isect: Coord[] = [];
...
export function simple_tri_aabb_isect(
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (let j = 0; j < 3; j++) {
      if (
        $vs_simple_tri_aabb_isect[j][i] < min[i] ||
        $vs_simple_tri_aabb_isect[j][i] >= max[i]
      )
        isect = false;
    }
...
export class MinMax {
  // When totaxis === 1 these hold scalars; otherwise they hold per-axis arrays.
  totaxis: number;
  min: number | Coord;
  max: number | Coord;
  _min: number | Coord;
  ...
  load(mm: MinMax) {
    if (this.totaxis === 1) {
      ...
      // Vector mode (totaxis !== 1): the source fields are per-axis arrays.
      this.min = new Vector3(mm.min as Coord);
      this.max = new Vector3(mm.max as Coord);
      this._min = new Vector3(mm._min as Coord);
      this._max = new Vector3(mm._max as Coord);
    }
  }
  ...
  reset() {
    ...
    if (totaxis === 1) {
      ...
      let _min = this._min as Coord,
        _max = this._max as Coord;
      let min = this.min as Coord,
        max = this.max as Coord;
  ...
  minmax_rect(p: Coord, size: Coord) {
    ...
    if (totaxis === 2) {
      ...
      throw (
        "Minmax.minmax_rect has no implementation for " +
        totaxis +
        "-dimensional data"
      );
    }
  ...
  minmax(p: number | Coord) {
    ...
    if (totaxis === 1) {
      ...
      let pv = p as Coord;
      if (totaxis === 2) {
        _min[0] = min[0] = Math.min(_min[0], pv[0]);
        _min[1] = min[1] = Math.min(_min[1], pv[1]);
        _max[0] = max[0] = Math.max(_max[0], pv[0]);
        _max[1] = max[1] = Math.max(_max[1], pv[1]);
      } else if (totaxis === 3) {
        _min[0] = min[0] = Math.min(_min[0], pv[0]);
        _min[1] = min[1] = Math.min(_min[1], pv[1]);
        _min[2] = min[2] = Math.min(_min[2], pv[2]);
        _max[0] = max[0] = Math.max(_max[0], pv[0]);
        _max[1] = max[1] = Math.max(_max[1], pv[1]);
        _max[2] = max[2] = Math.max(_max[2], pv[2]);
      } else {
        for (let i = 0; i < totaxis; i++) {
          _min[i] = min[i] = Math.min(_min[i], pv[i]);
          _max[i] = max[i] = Math.max(_max[i], pv[i]);
        }
      }
    }
  }

  // Serialization schema string, assigned just after the class declaration.
  static STRUCT: string;

  static fromSTRUCT(reader: (m: MinMax) => void) {
    let ret = new MinMax();
    reader(ret);
    return ret;
  }
...
export function inrect_2d(p: Coord, pos: Coord, size: Coord): boolean {
...
let $ps_aabb_isect_line_2d: Coord[] = [
  new Vector2(),
  new Vector2(),
  new Vector2(),
];
let $l1_aabb_isect_line_2d: Coord[] = [];
let $smax_aabb_isect_line_2d = new Vector2();
let $sv2_aabb_isect_line_2d = new Vector2();
let $l2_aabb_isect_line_2d: Coord[] = [];
...
export function aabb_isect_line_2d(
  ...
  if (
    !aabb_isect_2d(
      $smin_aabb_isect_line_2d,
      $smax_aabb_isect_line_2d,
      min,
      $ssize_aabb_isect_line_2d,
    )
  )
    return false;
  for (let i = 0; i < 4; i++) {
    if (inrect_2d(v1, min, $ssize_aabb_isect_line_2d)) return true;
    if (inrect_2d(v2, min, $ssize_aabb_isect_line_2d)) return true;
  }
  ...
  for (let i = 0; i < 4; i++) {
    let a = $ps_aabb_isect_line_2d[i],
      b = $ps_aabb_isect_line_2d[(i + 1) % 4];
...
export function expand_rect2d(pos: Coord, size: Coord, margin: Coord): void {
  pos[0] -= Math.floor(margin[0]);
  pos[1] -= Math.floor(margin[1]);
  size[0] += Math.floor(margin[0] * 2.0);
  size[1] += Math.floor(margin[1] * 2.0);
}
export function expand_line(l: Vector3[], margin: number): Vector3[] {
  let c = new Vector3();
  c.add(l[0]);
  c.add(l[1]);
  c.mulScalar(0.5);
...
export function colinear(a: Vector3, b: Vector3, c: Vector3): boolean {
  for (let i = 0; i < 3; i++) {
  ...
  if (
    _cross_vec1.dot(_cross_vec1) < limit ||
...
): Vector3 | typeof COLINEAR_ISECT | undefined {
...
/* This helper is called two ways in the codebase:
   - as four points (v1..v4), e.g. from line_line_isect, and
   - as two lines (v1, v2 are each a [point, point] pair, v3/v4 omitted),
     e.g. from convex_quad and aabb_isect_line_2d.
   The legacy body just feeds each argument to Vector3.load, so the param type
   covers both a single point and a line (array of points). */
type LineArg = Coord | Coord[];

export function line_line_cross(
  v1: LineArg,
  v2: LineArg,
  v3?: Coord,
  v4?: Coord,
  ...
  let l1 = _llc_l3,
    l2 = _llc_l4;
  l1[0].load(v1 as Coord),
    l1[1].load(v2 as Coord),
    l2[0].load(v3),
    l2[1].load(v4);
...
export function point_in_aabb_2d(p: Coord, min: Coord, max: Coord): boolean {
...
export function aabb_sphere_isect_2d(
  ...
  let ret =
    point_in_aabb_2d(v1, min, max) ||
    point_in_aabb_2d(v2, min, max) ||
...
export function point_in_aabb(p: Coord, min: Coord, max: Coord): boolean {
  return (
    p[0] >= min[0] &&
    p[0] <= max[0] &&
    p[1] >= min[1] &&
...
export function aabb_sphere_isect(
  ...
  return (
    point_in_aabb(v1, min, max) ||
    point_in_aabb(v2, min, max) ||
    point_in_aabb(v3, min, max)
  );
}

export function point_in_tri(
  p: Coord,
  v1: Coord,
  v2: Coord,
  v3: Coord,
...
export function normal_tri(v1: Coord, v2: Coord, v3: Coord): Vector3 {
  ...
  $e3_normal_tri[0] =
    $e1_normal_tri[1] * $e2_normal_tri[2] -
    $e1_normal_tri[2] * $e2_normal_tri[1];
  $e3_normal_tri[1] =
    $e1_normal_tri[2] * $e2_normal_tri[0] -
    $e1_normal_tri[0] * $e2_normal_tri[2];
  $e3_normal_tri[2] =
    $e1_normal_tri[0] * $e2_normal_tri[1] -
    $e1_normal_tri[1] * $e2_normal_tri[0];

  let _len = Math.sqrt(
    $e3_normal_tri[0] * $e3_normal_tri[0] +
      $e3_normal_tri[1] * $e3_normal_tri[1] +
      $e3_normal_tri[2] * $e3_normal_tri[2],
  );
...
export function line_isect(
  ...
  v4: Coord,
  calc_t?: boolean,
): [Vector3, number, number] | [Vector3, number] {
  if (calc_t === undefined) {
    calc_t = false;
  }
  let div =
...
export function dist_to_line(
  ...
  let lv1 = dt3l_v4.load(v1);
  let lv2 = dt3l_v5.load(v2);
...
export function clip_line_w(
  ...
  function doclip1(v1: Vector4, v2: Vector4, axis: number) {
    if (v1[axis] / v1[3] < -1) {
  ...
  function doclip(v1: Vector4, v2: Vector4, axis: number) {
    doclip1(v1, v2, axis);
    doclip1(v2, v1, axis);
  }

  // _unused matches a stray third argument passed at the call site below.
  function dozclip(v1: Vector4, v2: Vector4, _unused?: number) {
    if (v1[2] < 1) {
...
// Each slot holds [closest-point, t]. The point is filled in before return; the
// numeric placeholder is the initial fill only.
let _closest_point_on_line_cache = util.cachering.fromConstructor(Vector3, 64);
let _closest_point_rets = new util.cachering<[Vector3 | number, number]>(
  function (): [Vector3 | number, number] {
    return [0, 0];
  },
  64,
);
...
export function closest_point_on_line(
  ...
  let cp = _closest_point_on_line_cache.next();
  cp.load(l1).mulScalar(t).add(v1);
...
let _circ_from_line_tan_ret = new util.cachering<[Vector3, number]>(
  function (): [Vector3, number] {
    return [new Vector3(), 0];
  },
  32,
);

export function circ_from_line_tan(
  a: Coord,
  b: Coord,
  t: Coord,
): [Vector3, number] {
...
let _get_tri_circ_ret = new util.cachering<[Vector3 | number, number]>(
  function (): [Vector3 | number, number] {
    return [0, 0];
  },
  32,
...
//axis is optional, 0
export function rot2d(v1: Coord, A: number, axis?: number): void {
  let x = v1[0];
  let y = v1[1];
...
export function makeCircleMesh(
  gl: unknown,
  radius: number,
  stfeps: number,
): MeshLike {
...
export function minmax_verts(verts: Iterable<MeshVert>): [Vector3, Vector3] {
  ...
  let __iter_v = __get_iter<MeshVert>(verts);
  let v: MeshVert;
  while (1) {
