...
//      g.stroke();
  ...
  draw_sticks(g) {
    ...
    for (let si = 0; si < colors.colors.length; si++) {
      ...
      if (window.DRAW_TRANSPARENT) {
        alpha = ACCUM_ALPHA;
      }

      if (!window.SHOW_COLORS) {
        r1 = g1 = b1 = window.BLACK_BG ? 255 : 1.0;
      }
  ...
  draw(g) {
    ...
    if (window.RASTER_IMAGE && this.raster_image !== undefined) {
      g.putImageData(this.raster_image, 20, 20);
    } else if (window.DRAW_STICKS) {
      this.draw_transform(g);
      this.draw_sticks(g);
    } else if (window.TRI_MODE) {
      this.draw_transform(g);
